/** Shared admin panel design tokens. */
export const ADMIN_EMPTY = 'None'

export const ADMIN_PANEL_CLASS = 'admin-panel'

export const ADMIN_CARD_CLASS =
  'admin-card !bg-white !border !border-[#eef1f5] !shadow-none rounded-2xl overflow-hidden'

export const ADMIN_STATUS_HEADER_CLASS =
  'admin-status-header bg-white rounded-2xl px-6 py-5 border border-[#eef1f5]'

export const ADMIN_STATUS_POPOVER_CLASS = 'admin-status-popover'

/** Vertical spacing between stacked admin cards. */
export const ADMIN_STACK_SPACE = 'admin-stack'

/** Grid gap between admin cards (pair with grid column classes). */
export const ADMIN_STACK_GAP = 'admin-grid'

export const ADMIN_CARD_HEAD_CLASS =
  '!border-b !border-[#eef1f5] !px-6 !py-4 !min-h-0 admin-card-head'

export const ADMIN_CARD_BODY_CLASS = '!px-6 !py-5'

export const ADMIN_PAGE_CLASS = 'flex h-full min-h-0 min-w-0 bg-[#f4f6f9]'

export const ADMIN_SECTION_NAV_CLASS =
  'w-[220px] shrink-0 border-r border-[#e8edf2] bg-white flex flex-col'

export const ADMIN_SECTION_MAIN_CLASS =
  'flex-1 min-w-0 overflow-y-auto overflow-x-hidden p-6 md:p-8 bg-[#f4f6f9]'

export const ADMIN_PAGE_TITLE = 'admin-page-title'

export const ADMIN_PAGE_SUBTITLE = 'admin-page-subtitle'

export const ADMIN_TEXT_LABEL = 'admin-text-label'

export const ADMIN_TEXT_BODY = 'admin-text-body'

export const ADMIN_TEXT_MUTED = 'admin-text-muted'

export const ADMIN_TEXT_DESC = 'admin-text-desc'

export const ADMIN_TEXT_MONO = 'admin-text-mono'

export const ADMIN_TEXT_LINK = 'admin-text-link'

export const ADMIN_TEXT_EMPHASIS = 'admin-text-emphasis'

export const ADMIN_TEXT_ERROR = 'admin-text-error'

export const ADMIN_STAT_VALUE =
  '[&_.ant-statistic-content-value]:!text-2xl [&_.ant-statistic-content-value]:!font-semibold [&_.ant-statistic-content-value]:!text-[#1a2332]'

export const ADMIN_STAT_TITLE = 'admin-stat-title'

export const ADMIN_TABLE_SCROLL = { x: 960 } as const
